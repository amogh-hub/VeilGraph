...
def _v3_address_spans(line: PositionedLine) -> list[_Span]:
    ...
    if postal_context:
        for match in _V3_POSTAL_TOKEN_RE.finditer(text, postal_context.end()):
            ...
            break
    structural_address_signal = strong_address_context or any(
        span.entity_type in {EntityType.BUILDING_NUMBER, EntityType.STREET_ADDRESS, EntityType.LOCALITY}
        for span in result
    )
    if structural_address_signal:
