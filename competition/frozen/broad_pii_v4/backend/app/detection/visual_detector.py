...
def _signature_candidates(page: PageFrame) -> list[DetectedMention]:
    ...
    for candidate_index, line in enumerate(keyword_lines):
        if not line.tokens:
            continue
        # OCR can merge the handwritten mark into the same text line (for
        # example ``Signature candidate: ee``).  Geometry for the search anchor
        # must therefore stop at the actual label tokens rather than expanding
        # across OCR noise produced by the signature itself.
        keyword_indexes = [
            i for i, token in enumerate(line.tokens)
            if re.search(r"signature|signed|signatory", token.text, re.IGNORECASE)
        ]
        label_end = max(keyword_indexes) if keyword_indexes else 0
        while label_end + 1 < len(line.tokens) and re.search(
            r"^(?:candidate|field|area)[:.-]?$", line.tokens[label_end + 1].text, re.IGNORECASE
        ):
            label_end += 1
        label_tokens = line.tokens[: label_end + 1]
        label_x0 = min(token.x0 for token in label_tokens)
        label_x1 = max(token.x1 for token in label_tokens)
        label_y0 = min(token.y0 for token in label_tokens)
        label_y1 = max(token.y1 for token in label_tokens)
        # Signature fields occur both below a label and on the same row to its
        # right.  Search both bounded regions and retain the strongest ink object.
        regions = [
            (
                max(0, int((label_x1 + 8) * scale_x)),
                max(0, int((label_y0 - 10) * scale_y)),
                min(page.image.width, int((page.width - 24) * scale_x)),
                min(page.image.height, int((label_y1 + 52) * scale_y)),
            ),
            (
                max(0, int(label_x0 * scale_x)),
                max(0, int((label_y1 + 2) * scale_y)),
                min(page.image.width, int((page.width - 24) * scale_x)),
                min(page.image.height, int((label_y1 + 76) * scale_y)),
            ),
        ]
        ...
        for x0, y0, x1, y1 in regions:
            ...
            if width < 55 or height < 8 or ink_pixels < 80:
                continue
            rect = (
                float(max(0, x0 + min_x - 4)),
                float(max(0, y0 + min_y - 4)),
                float(min(page.image.width, x0 + max_x + 4)),
                float(min(page.image.height, y0 + max_y + 4)),
            )
            score = ink_pixels + width * 2 + height
