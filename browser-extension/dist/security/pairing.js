...
export async function pairLocalCompanion(nowMs) {
    ...
    const publicKey = await crypto.subtle.importKey('raw', base64ToBytes(raw.payload.signer.public_key_b64), { name: 'Ed25519' }, false, ['verify']);
    const signatureValid = await crypto.subtle.verify({ name: 'Ed25519' }, publicKey, base64ToBytes(raw.signature_b64), new TextEncoder().encode(canonicalJson(raw.payload)));
    if (!signatureValid)
        throw new Error('local companion pairing signature is invalid');
...
export async function signerMatchesPinnedTrust(publicKeyB64, fingerprint) {
    ...
    if (!trusted)
        return false;
    if (trusted.publicKeyB64 !== publicKeyB64 || trusted.publicKeySha256 !== fingerprint)
        return false;
