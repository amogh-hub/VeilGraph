...
export async function verifyNetworkAuthorization(authorization, payload, nowMs = Date.now()) {
    if (payload.schema !== BROWSER_RELEASE_SCHEMA)
        return { allowed: false, reason: 'unsupported release payload schema' };
    if (authorization.payload.schema !== BROWSER_AUTH_SCHEMA)
        return { allowed: false, reason: 'unsupported authorization schema' };
    if (authorization.signature_algorithm !== 'Ed25519')
        return { allowed: false, reason: 'unsupported signature algorithm' };
    if (authorization.payload.decision !== 'ALLOW_NETWORK_RELEASE')
        return { allowed: false, reason: 'local privacy gate denied network release' };
    if (authorization.payload.critical_failures !== 0)
        return { allowed: false, reason: 'critical privacy blocker recorded' };
    if (authorization.payload.proof_score !== 100)
        return { allowed: false, reason: 'browser privacy proof is incomplete' };
    ...
    if (payload.privacy_level < payload.network_privacy_floor)
        return { allowed: false, reason: 'network privacy floor is not satisfied' };
    ...
    try {
        const publicKey = await crypto.subtle.importKey('raw', base64ToBytes(authorization.payload.signer.public_key_b64), { name: 'Ed25519' }, false, ['verify']);
        const verified = await crypto.subtle.verify({ name: 'Ed25519' }, publicKey, base64ToBytes(authorization.signature_b64), new TextEncoder().encode(canonicalJson(authorization.payload)));
...
export async function verifyTrustedNetworkAuthorization(authorization, payload, nowMs = Date.now()) {
    const pinned = await signerMatchesPinnedTrust(authorization.payload.signer.public_key_b64, authorization.payload.signer.public_key_sha256);
    if (!pinned)
        return { allowed: false, reason: 'authorization signer does not match the explicitly paired local VeilGraph companion' };
