...
function liveSnapshot(element) {
    ...
    const control = (element instanceof HTMLButtonElement
...
function parseBooleanSelection(value) {
    ...
    if (['true', 'on', 'yes', 'checked', '1'].includes(normalized))
        return true;
    if (['false', 'off', 'no', 'unchecked', '0'].includes(normalized))
        return false;
    return null;
}
function executeSelect(element, value) {
    if (element instanceof HTMLSelectElement) {
        ...
        if (!option)
            throw new Error('requested select option is not present in the live control');
    ...
    if (element instanceof HTMLInputElement && (element.type === 'checkbox' || element.type === 'radio')) {
        ...
        if (desired === null)
            throw new Error('checkbox/radio selection value must be a boolean-like value');
...
export function executeFrameAction(executionId, action) {
    if (action.action === 'SCROLL') {
        const delta = action.scroll_delta_y;
        if (!delta)
            throw new Error('SCROLL action is missing delta');
    ...
    if (!action.target_id)
        throw new Error(`${action.action} requires a local target`);
    const element = resolveLocalElement(action.target_id);
    if (!element)
        throw new Error('target DOM node is stale or no longer available');
    assertLiveElementSecurity(action, liveSnapshot(element));
    if (action.action === 'CLICK') {
        if (!(element instanceof HTMLElement))
            throw new Error('CLICK target is not an HTMLElement');
        element.focus({ preventScroll: true });
        element.click();
    }
    else if (action.action === 'TYPE') {
        if (action.value === null || action.value === undefined)
            throw new Error('TYPE action is missing value');
        executeType(element, action.value);
    }
    else if (action.action === 'SELECT') {
        if (action.value === null || action.value === undefined)
            throw new Error('SELECT action is missing value');
