...
const loopButton = document.querySelector('#agent-loop');
...
function setBusy(busy) {
    ...
    if (executeButton)
        executeButton.disabled = busy || pendingExecution === null || loopRunning;
    if (loopButton)
        loopButton.disabled = busy || loopRunning;
    if (stopLoopButton)
        stopLoopButton.disabled = !loopRunning || activeLoopId === null;
    if (pairButton)
...
async function prepareRelease() {
    ...
    try {
        ...
        detail.textContent = JSON.stringify({
            ...
            terminalEvidence: result.payload.terminal_evidence,
...
async function reasonSafely() {
    ...
    try {
        ...
        detail.textContent = JSON.stringify({
            ...
            terminalEvidence: data.preparation.payload.terminal_evidence,
...
async function continueLoopFromResult(result) {
    renderLoop(result);
    if (result.status !== 'WAITING_CONFIRMATION' || !result.pending_execution || !activeLoopId)
        return;
    ...
    if (!response.ok)
        throw new Error(response.error);
    await continueLoopFromResult(response.data);
}
...
async function stopSecureAgentLoop() {
    if (!activeLoopId || !status)
        return;
    const loopId = activeLoopId;
