...
const TARGET_ROLES: Record<string, Set<string>> = {
...
function record(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('reasoning response must be an object')
...
function targetText(payload: BrowserReleasePayload, action: BrowserAction): string {
...
function highImpact(payload: BrowserReleasePayload, action: BrowserAction): boolean {
...
function validateActionShape(action: BrowserAction): void {
  if (!ACTIONS.has(action.action)) throw new Error(`unsupported action ${String(action.action)}`)
  ...
  if (!action.reason || action.reason.length > 300) throw new Error('action reason is invalid')
}

export function validateReasoningResponse(raw: unknown, payload: BrowserReleasePayload, expectedPayloadSha256: string): BrowserReasoningResponse {
  const response = record(raw)
  if (response.schema !== 'veilgraph.browser-reasoning-response.v1') throw new Error('unsupported reasoning response schema')

  const plan = record(response.plan)
  if (plan.schema !== 'veilgraph.browser-action-plan.v1') throw new Error('unsupported action-plan schema')
  ...
  for (const rawAction of plan.actions) {
    const action = record(rawAction) as unknown as BrowserAction
    validateActionShape(action)

    if (action.target_id) {
      const target = elements.get(action.target_id)
      if (!target) throw new Error(`reasoning server invented target ${action.target_id}`)
      if (target.disabled && action.action !== 'READ') throw new Error(`reasoning server targeted disabled element ${action.target_id}`)
  ...
  if (
    evidence.authorization_verified !== true
  ...
  return raw as BrowserReasoningResponse
}
