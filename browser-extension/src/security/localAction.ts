...
export interface PendingExecutionRecord {
  public: PendingActionExecution
  tabId: number
  payloadOrigin: string
  frameOrigin: string | null
  releasedTargetRole: string | null
}
...
function requiresLocalConfirmation(payload: BrowserReleasePayload, action: BrowserAction): boolean {
...
function containsDirectIdentifier(value: string | null | undefined): boolean {
...
export function createPendingExecution(
  ...
  nowMs: number,
): PendingExecutionRecord | null {
  if (reasoning.plan.complete || reasoning.plan.actions.length === 0) return null
  ...
  let releasedTargetRole: string | null = null

  if (action.target_id) {
    const released = payload.page.elements.find((element) => element.element_id === action.target_id)
    if (!released) throw new Error(`target ${action.target_id} is absent from released context`)
    const frame = frames.find((candidate) => candidate.elements.some((element) => element.localId === action.target_id))
    if (!frame) throw new Error(`target ${action.target_id} is absent from original local capture`)
    frameId = frame.frameId
    frameOrigin = frame.origin
    releasedTargetRole = released.role
  } else if (action.action === 'SCROLL') {
    const top = frames.find((frame) => frame.isTopFrame)
    if (!top) throw new Error('top frame is unavailable for scroll execution')
    frameId = top.frameId
...
export function validatePendingExecution(
  record: PendingExecutionRecord,
  activeTabId: number,
  activeOrigin: string,
  freshFrame: FrameCapture | null,
  confirmed: boolean,
  nowMs: number,
): { frameId: number | null } {
  if (activeTabId !== record.tabId) throw new Error('active tab changed after reasoning')
  if (!sameOrigin(activeOrigin, record.payloadOrigin)) throw new Error('page origin changed after reasoning')
  if (nowMs > Date.parse(record.public.expires_at)) throw new Error('pending action expired')
  if (record.public.requires_confirmation && !confirmed) throw new Error('high-impact action requires explicit local confirmation')
  ...
  if (action.action === 'WAIT') return { frameId: null }

  const frameId = record.public.frame_id
  if (frameId === null || freshFrame === null) throw new Error('fresh frame evidence is required before local execution')
  if (freshFrame.frameId !== frameId) throw new Error('fresh frame identity does not match pending action')
  ...
  if (action.target_id) {
    ...
    if (!freshTarget) throw new Error('target DOM node is stale, hidden, offscreen, replaced, or missing')
    if (freshTarget.disabled) throw new Error('target became disabled after reasoning')
