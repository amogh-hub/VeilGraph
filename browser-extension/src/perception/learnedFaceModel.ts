import type { LearnedModelEvidence, VisualCapability, VisualFinding } from '../common/protocol.js'
...
type TensorData = Float32Array | Int32Array | Uint8Array | readonly number[]
...
export interface LearnedFaceDetectionResult {
  findings: VisualFinding[]
  capability: VisualCapability
  evidence: LearnedModelEvidence
}

export interface LearnedFaceDetector {
  detect(bitmap: ImageBitmap): Promise<LearnedFaceDetectionResult>
}
...
async function loadVerifiedModel(resolveAssetUrl: (path: string) => string): Promise<Uint8Array> {
  const response = await fetch(resolveAssetUrl(ULTRAFACE_MODEL_PATH), {
    cache: 'no-store',
    credentials: 'omit',
    redirect: 'error',
  })
  if (!response.ok) throw new Error(`packaged model fetch failed (${response.status})`)
...
function preprocess(bitmap: ImageBitmap): Float32Array {
  ...
  if (!context) throw new Error('2D canvas context unavailable for learned-model preprocessing')
...
function tensorByLastDimension(
  outputs: Record<string, RuntimeTensor>,
  outputNames: readonly string[],
  lastDimension: number,
): RuntimeTensor | undefined {
  return outputNames
    .map((name) => outputs[name])
    .find((tensor): tensor is RuntimeTensor => Boolean(tensor && tensor.dims[tensor.dims.length - 1] === lastDimension))
}
...
function hardNms(candidates: CandidateFace[]): CandidateFace[] {
  const sorted = [...candidates].sort((left, right) => right.confidence - left.confidence)
  const kept: CandidateFace[] = []
  for (const candidate of sorted) {
    if (kept.some((existing) => iou(existing.bbox, candidate.bbox) > NMS_IOU_THRESHOLD)) continue
    kept.push(candidate)
...
function postprocess(session: RuntimeSession, outputs: Record<string, RuntimeTensor>): VisualFinding[] {
  ...
  if (!scores || !boxes) throw new Error('UltraFace output tensors are missing score or box dimensions')

  const scoreData = scores.data
  const boxData = boxes.data
  const candidateCount = Math.min(
    Math.floor(scoreData.length / 2),
    Math.floor(boxData.length / 4),
  )
  const candidates: CandidateFace[] = []
  for (let index = 0; index < candidateCount; index += 1) {
    const confidence = Number(scoreData[index * 2 + 1] ?? 0)
    if (!Number.isFinite(confidence) || confidence < FACE_THRESHOLD) continue
...
function unavailableEvidence(reason: string, modelLoadMs: number): LearnedModelEvidence {
  return {
 ...
 */
export function createOnnxLearnedFaceDetector(
  ort: OnnxRuntimeLike,
  resolveAssetUrl: (path: string) => string,
): LearnedFaceDetector {
  let modelPromise: Promise<Uint8Array> | null = null
  let modelLoadMs = 0
  const sessions = new Map<ExecutionProvider, Promise<RuntimeSession>>()

  const model = async (): Promise<Uint8Array> => {
    if (!modelPromise) {
      const started = nowMs()
  ...
  const session = async (provider: ExecutionProvider): Promise<RuntimeSession> => {
  ...
  return {
    async detect(bitmap: ImageBitmap): Promise<LearnedFaceDetectionResult> {
      try {
...
export function unavailableLearnedFaceResult(detail: string): LearnedFaceDetectionResult {
  return {
    findings: [],
    capability: {
      name: 'LEARNED_FACE_DETECTION',
