...
interface DetectionRectLike {
  boundingBox: DOMRectReadOnly
  rawValue?: string
}

interface NativeFaceDetector {
  detect(source: ImageBitmapSource): Promise<DetectionRectLike[]>
}
...
interface NativeBarcodeDetector {
  detect(source: ImageBitmapSource): Promise<Array<DetectionRectLike & { rawValue: string }>>
}

type ShapeDetectionGlobal = typeof globalThis & {
...
function rectToBasisPoints(rect: DOMRectReadOnly, width: number, height: number): [number, number, number, number] {
  return [
...
function fuseCrossModalSensitiveRegions(findings: VisualFinding[]): VisualFinding[] {
  ...
  const fused = findings.map((finding) => {
    if (finding.type !== 'PASSWORD_FIELD' && finding.type !== 'SENSITIVE_REGION') return finding
    let merged = finding
...
async function decodeScreenshot(dataUrl: string): Promise<ImageBitmap> {
  if (!dataUrl.startsWith('data:image/')) throw new Error('visual perception received a non-image data URL')
  const response = await fetch(dataUrl)
  if (!response.ok) throw new Error(`failed to decode screenshot (${response.status})`)
  return createImageBitmap(await response.blob())
}

function projectDomSensitiveRegions(frames: FrameCapture[]): VisualFinding[] {
  ...
  for (const element of topFrame.elements) {
    ...
    if (!isPassword && matchedHints.length === 0) continue
    findings.push({
...
async function nativeBarcodeFindings(bitmap: ImageBitmap): Promise<{ findings: VisualFinding[]; capability: VisualCapability }> {
  const scope = globalThis as ShapeDetectionGlobal
...
async function nativeFaceFindings(bitmap: ImageBitmap): Promise<{ findings: VisualFinding[]; capability: VisualCapability }> {
  const scope = globalThis as ShapeDetectionGlobal
...
async function nativeTextFindings(bitmap: ImageBitmap): Promise<{ findings: VisualFinding[]; capability: VisualCapability }> {
  const scope = globalThis as ShapeDetectionGlobal
  if (!scope.TextDetector) return canvasTextRegionFindings(bitmap)
  try {
...
async function canvasTextRegionFindings(bitmap: ImageBitmap): Promise<{ findings: VisualFinding[]; capability: VisualCapability }> {
  try {
    const maxWidth = 960
    ...
    const luminance = (offset: number): number => Math.round(0.2126 * data[offset]! + 0.7152 * data[offset + 1]! + 0.0722 * data[offset + 2]!)
    ...
    for (let y = 0; y < height; y += 1) {
      ...
      if (active) {
        ...
        quietRows += 1
        if (quietRows > 2) close(y - quietRows)
      }
      if (regions.length >= MAX_TEXT_REGIONS) break
    }
    if (start >= 0 && regions.length < MAX_TEXT_REGIONS) close(height - 1)
...
function deriveStatus(capabilities: VisualCapability[]): LocalPerceptionStatus {
  const required = capabilities.filter((item) => item.required)
  if (required.every((item) => item.status === 'READY')) return 'READY'
  if (required.some((item) => item.status === 'ERROR')) return 'PARTIAL'
